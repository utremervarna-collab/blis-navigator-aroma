...
(function(){
  ...
  function renderSoon(delay=60){
    setTimeout(()=>{try{if(document.getElementById('digital')?.classList.contains('active'))window.BLISDigitalRadar?.render?.()}catch(e){}},delay);
  }
  function wrapRefGo(){
    const old=window.refGo;
    if(typeof old!=='function'||old.__digitalRadarBoot)return false;
  ...
  document.addEventListener('click',e=>{
    if(e.target?.closest?.('#nav button[data-page="digital"]')){renderSoon(40);renderSoon(220)}
  },true);
  document.getElementById('clientSel')?.addEventListener('change',()=>renderSoon(180));

  function init(){
    wrapRefGo();
    if(document.getElementById('digital')?.classList.contains('active'))renderSoon(20);
