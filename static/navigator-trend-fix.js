/* BLIS Navigator — real daily trend correction. Uses the recorded BLIS index from each daily snapshot; no frontend re-weighting. */
(function(){
  ...
  function activityByDay(){
    const out=new Map();
    let activity=[];try{activity=window.BLISPeriod?.activities?BLISPeriod.activities():(Array.isArray(A)?A:[])}catch(e){}
  ...
  function daily(){
    let list=[];
    try{list=window.BLISPeriod?.snapshots?BLISPeriod.snapshots():(Array.isArray(H)?H.slice():[])}catch(e){return[]}
    const byDay=new Map();
    list.slice().sort((a,b)=>new Date(a?.created_at||a?.observed_at||a?.time||0)-new Date(b?.created_at||b?.observed_at||b?.time||0)).forEach(s=>{
      const d=dayOf(s);if(!d)return;
      const v=N(s?.payload?.blis_index);if(v==null)return;
      /* Last valid snapshot for each calendar day is the official daily BLIS value. */
      byDay.set(d,{date:d,value:Math.round(clamp(v)*10)/10});
    });
    const counts=activityByDay();
    return [...byDay.values()].map(x=>({...x,activity:counts.get(x.date)||0}));
  }

  function smoothPath(points){
    if(points.length<2)return'';
    if(points.length===2){const [a,b]=points,m=(a[0]+b[0])/2;return `M ${a[0]} ${a[1]} C ${m} ${a[1]}, ${m} ${b[1]}, ${b[0]} ${b[1]}`;}
  ...
  function draw(){
    const host=document.querySelector('#overviewPremium .ov-trend');if(!host)return;
    const s=daily();
    if(s.length<2){
      const days=window.BLISPeriod?.days||30;
      host.innerHTML=`<div class="ov-no-data">${s.length===1?`Има 1 реална дневна BLIS точка за избраните ${days} дни.`:`Все още няма дневна BLIS история за избраните ${days} дни.`}</div>`;
      return;
    }
    const a=s.map(x=>x.value),w=620,h=246,l=34,r=18,t=16,b=52;
    const times=s.map(x=>new Date(x.date+'T00:00:00Z').getTime()),minT=Math.min(...times),maxT=Math.max(...times),span=Math.max(86400000,maxT-minT);
    const x=i=>l+(times[i]-minT)*(w-l-r)/span,y=v=>t+(100-clamp(v))*(h-t-b)/100;
    const pts=a.map((v,i)=>[x(i),y(v)]),line=smoothPath(pts),area=`M ${pts[0][0]} ${h-b} L ${pts.map(p=>p.join(' ')).join(' L ')} L ${pts[pts.length-1][0]} ${h-b} Z`;
    ...
    const oldNote=host.parentElement.querySelector(':scope > .ov-method-note');if(oldNote)oldNote.remove();
  }
  ...
  window.addEventListener('blis:periodchange',schedule);window.addEventListener('blis:clientdata',schedule);
