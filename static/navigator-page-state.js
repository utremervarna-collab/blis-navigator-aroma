/* BLIS Navigator — canonical persistent page state v4. */
(function(){
'use strict';
if(window.__BLIS_PAGE_STATE_V4)return;window.__BLIS_PAGE_STATE_V4=true;
...
function wrap(){const fn=window.refGo;if(typeof fn!=='function'||fn.__blisStateV4)return;function w(p,...args){p=PAGES.has(String(p))?String(p):'overview';if(!internal)save(p,true);return fn.call(this,p,...args)}w.__blisStateV4=true;w.__base=fn;window.refGo=w}
function restore(){wrap();const p=wanted();if(active()!==p)go(p);save(p,true)}
document.addEventListener('click',e=>{const b=e.target.closest?.('#nav button[data-page],[data-blis-commerce-open]');if(b){const p=b.matches?.('[data-blis-commerce-open]')?'commerce':b.dataset.page;if(PAGES.has(p))save(p,true)}const o=e.target.closest?.('.client-option[data-client-key]');if(o&&CLIENTS.has(o.dataset.clientKey)){try{localStorage.setItem(KEY+o.dataset.clientKey,active())}catch(_){}}},true);
