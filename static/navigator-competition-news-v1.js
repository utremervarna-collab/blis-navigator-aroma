...
let apiRows=[],apiClient='',apiUpdated=0,apiBusy=false,apiError=false,syncTimer=0,renderTimer=0,lastRenderSignature='';
...
for(const ev of ['blis:clientdata','blis:intelligence','blis:periodchange','blis:routechange'])window.addEventListener(ev,()=>{schedule(120);resync(ev==='blis:clientdata')});
...
function boot(){css();cleanMarket();schedule(250);syncAPI(true);setTimeout(()=>schedule(0),900);setTimeout(cleanMarket,2200);setInterval(()=>{if(document.visibilityState!=='hidden')syncAPI(false)},60000)}
