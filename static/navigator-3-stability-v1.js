/* BLIS Navigator 3.0 — visual owner stability v1.
...
function ensure(id){if(id==='overview')ensureOverview();else if(id==='social')ensureSocial();else if(id==='market')ensureMarket();else if(id==='competition')ensureCompetition();else if(id==='history')ensureHistory();document.documentElement.dataset.navigatorVisualStability='v1'}
function active(){return document.querySelector('.page.active')?.id||new URLSearchParams(location.search).get('page')||'overview'}
function schedule(id=active()){ensure(id);setTimeout(()=>ensure(id),70);setTimeout(()=>ensure(id),220)}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>schedule(active()),{once:true});else schedule(active());
window.BLISNavigator3StabilityV1={ensure,schedule};
})();
