/* BLIS Navigator — Reputation bootstrap v33. Stable, single-pass controller. */
(function(){
'use strict';
...
function run(delay=80,force=false){setTimeout(()=>{if(active())window.BLISReputationStableV33?.run?.(force)},delay)}
function wrapRefGo(){const old=window.refGo;if(typeof old!=='function'||old.__repStable33)return false;const wrapped=function(id){const r=old.apply(this,arguments);if(id==='reputation')run(90,false);return r};wrapped.__repStable33=true;window.refGo=wrapped;return true}
function init(){
 ...
 script('data-blis-reputation-stable33','/navigator-reputation-stable-v33.js?v=20260819-reputation33',()=>run(70,false));
 wrapRefGo();
