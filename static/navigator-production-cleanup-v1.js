/* BLIS Navigator — production cleanup guard v1.
   Canonicalizes navigation, client switching and chart marker behavior without polling.
   Loaded last by dashboard.html. */
(function(){
'use strict';
if(window.__BLIS_PRODUCTION_CLEANUP_V1)return;window.__BLIS_PRODUCTION_CLEANUP_V1=true;
...
function normalizeNav(){
  ...
  [...nav.querySelectorAll('button[data-page]')].forEach(b=>{
    const id=String(b.dataset.page||'');
    if(!PAGES.has(id))b.remove();
  });
  ...
  Object.entries(labels).forEach(([id,label])=>{const b=nav.querySelector(`button[data-page="${id}"]`);const t=b?.querySelector('.navtxt')||b?.querySelector('span:last-child');if(t&&t.textContent!==label)t.textContent=label});
}
...
if(main)new MutationObserver(ms=>{if(ms.some(m=>[...m.addedNodes].some(n=>n.nodeType===1)))schedule()}).observe(main,{childList:true,subtree:true});
})();
