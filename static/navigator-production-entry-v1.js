/* BLIS Navigator — production entrypoint v37.
   Name-only client identity + Overview marker locked to curve + canonical visual owners. */
(function(){
'use strict';
if(window.__BLIS_PRODUCTION_ENTRY_V37)return;window.__BLIS_PRODUCTION_ENTRY_V37=true;
const VERSION='20260830-client-name-only-1';
...
async function boot(){
 ...
 document.documentElement.dataset.navigatorUi='client-name-only-1';
 ...
 setTimeout(()=>{window.BLISClientBrandingV4?.paint?.();window.BLISOverviewMarkerFixV1?.align?.();window.BLISColorSystemV1?.decorate?.();window.BLISNoPageNumbersV1?.clean?.();window.BLISLanguageCleanupV1?.clean?.()},100);
}
