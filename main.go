...
type Store struct { Clients map[string]*Client `json:"clients"` }
...
func appDataDir() string {
    if p:=os.Getenv("DATA_DIR"); p!="" { return p }
    if p:=os.Getenv("APPDATA"); p!="" { return filepath.Join(p,"BLIS Navigator") }
    if os.Getenv("RENDER")!="" { return "/tmp/blis-navigator" }
    h,_:=os.UserHomeDir(); return filepath.Join(h,".blis-navigator")
}
func latest(c *Client, source, metric string) interface{} {
    for i:=len(c.Observations)-1;i>=0;i-- {
        o:=c.Observations[i]
        if o.SourceKey==source && o.MetricKey==metric { return o.Value }
    }
    return nil
}
func f(v interface{}) float64 {
    ...
    case json.Number:y,_:=x.Float64();return y
    }
    return 0
}
...
func mean(xs []float64) float64 { if len(xs)==0{return 0};s:=0.0;for _,x:=range xs{s+=x};return s/float64(len(xs)) }
func add(c *Client,s,m string,v interface{},stamp string){c.Observations=append(c.Observations,Observation{SourceKey:s,MetricKey:m,Value:v,ObservedAt:stamp})}
...
func ensureStore(){
    ...
    if b,err:=os.ReadFile(dataPath);err==nil{
        if json.Unmarshal(b,&store)==nil && len(store.Clients)>0{return}
    }
    store=seedStore();saveStore()
}
...
func sig(level,title,text string) map[string]interface{} { return map[string]interface{}{"level":level,"title":title,"text":text} }
...
func astorDashboard(c *Client) map[string]interface{} {
    ...
    m:=mean(norm);sd:=0.0;for _,v:=range norm{sd+=(v-m)*(v-m)};sd=math.Sqrt(sd/3)
...
func dashboard(c *Client) map[string]interface{} { if c.Slug=="astor-garden"{return astorDashboard(c)};return aromaDashboard(c) }

func probeOfficial(c *Client) map[string]interface{} {
    var src *Source
    for i:=range c.Sources{if c.Sources[i].Key=="official_site"{src=&c.Sources[i];break}}
    if src==nil{return map[string]interface{}{"ok":false}}
    cli:=http.Client{Timeout:12*time.Second}
    req,_:=http.NewRequest("GET",src.URL,nil);req.Header.Set("User-Agent","BLIS-Navigator/1.2")
    resp,err:=cli.Do(req);if err!=nil{return map[string]interface{}{"ok":false,"message":err.Error()}}
    defer resp.Body.Close()
    _,_=io.ReadAll(io.LimitReader(resp.Body,1024*1024))
    add(c,"official_site","website_active",func()float64{if resp.StatusCode<400{return 1};return 0}(),nowISO())
...
func jsonOut(w http.ResponseWriter,v interface{}){w.Header().Set("Content-Type","application/json; charset=utf-8");_ = json.NewEncoder(w).Encode(v)}
func handler(w http.ResponseWriter,r *http.Request){
    path:=strings.Trim(r.URL.Path,"/")
    if path==""{w.Header().Set("Content-Type","text/html; charset=utf-8");io.WriteString(w,indexHTML);return}
    if path=="api/health"{jsonOut(w,map[string]interface{}{"ok":true,"time":nowISO()});return}
    if path=="api/clients"{
    ...
    if len(p)>=5&&p[0]=="api"&&p[1]=="clients"&&p[3]=="reports"&&p[4]!=""{
        ...
        if title==""{http.NotFound(w,r);return}
    ...
    if len(p)>=4&&p[0]=="api"&&p[1]=="clients"{
        c,ok:=store.Clients[p[2]];if !ok{http.NotFound(w,r);return}
        switch p[3]{
        case "dashboard":jsonOut(w,dashboard(c));return
        case "sources":jsonOut(w,c.Sources);return
        case "history":jsonOut(w,c.Snapshots);return
        case "reports":
        ...
        case "refresh":
            ...
            mu.Lock();res:=probeOfficial(c);d:=dashboard(c);c.Snapshots=append(c.Snapshots,Snapshot{CreatedAt:nowISO(),Payload:d});saveStore();mu.Unlock()
            jsonOut(w,map[string]interface{}{"ok":true,"connector":res,"dashboard":d});return
        }
    }
