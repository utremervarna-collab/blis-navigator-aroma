...
function setLabel(t,c){
 ...
 if(label.dataset.client!==c||label.textContent!==(NAMES[c]||c))label.textContent=NAMES[c]||c;
...
async function doApply(){
 ...
 try{
   ...
   if(typeof img.decode==='function'){
     try{await img.decode()}catch(_){if(!img.complete)throw _}
   }else if(!img.complete){
     await new Promise((resolve,reject)=>{img.addEventListener('load',resolve,{once:true});img.addEventListener('error',()=>reject(new Error('image decode failed')),{once:true})});
   }
...
function init(){
 ...
 if(root&&!observer){
   observer=new MutationObserver(()=>{
     if(root.querySelector('.rp-totem')){schedule();observer?.disconnect();observer=null}
   });
   if(!root.querySelector('.rp-totem'))observer.observe(root,{childList:true,subtree:true});
 }
