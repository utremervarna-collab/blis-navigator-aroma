...
'публично индексирано web съдържание':'publicly indexed web content',
'се събират автоматично.':'are collected automatically.',
'Закрити групи':'Closed groups','непублични профили':'non-public profiles','лицензиран доставчик':'licensed provider',
'КУБ Корпорация':'KUB Corporation','KYB Corporation':'KUB Corporation','KYB / KUB':'KUB / KUB'
};
const RU={
'Покритие: новини':'Охват: новости',
...
'публично индексирано web съдържание':'публично индексируемый web-контент','се събират автоматично.':'собираются автоматически.','Закрити групи':'Закрытые группы','непублични профили':'непубличные профили','лицензиран доставчик':'лицензированный поставщик'
};
...
function boot(){apply(document.body);[100,350,900,1800].forEach(x=>setTimeout(()=>apply(document.body),x));const mo=new MutationObserver(ms=>{if(busy)return;for(const m of ms){const p=m.target.nodeType===1?m.target:m.target.parentElement;if(p&&p.closest&&p.closest('#sources,#settings,#stakeholders,#evidence')){setTimeout(()=>apply(p.closest('.page')||document.body),30);break}}});mo.observe(document.body,{subtree:true,childList:true,characterData:true});document.addEventListener('click',e=>{if(e.target.closest('#nav'))[40,200,700].forEach(x=>setTimeout(()=>apply(document.body),x))},true)}
