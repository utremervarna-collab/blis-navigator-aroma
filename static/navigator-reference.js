/* BLIS Navigator — canonical production router v6. One route owner + persistent delegated nav. */
(function(){
'use strict';
if(window.__BLIS_REFERENCE_V6)return;window.__BLIS_REFERENCE_V6=true;
...
function nav(){const n=document.getElementById('nav');if(!n)return;const active=document.querySelector('.page.active')?.id||'overview',sig=NAV.map(([id,icon,label])=>`${id}|${icon}|${label}|${id===active?'1':'0'}`).join(';');if(n.dataset.refv6Sig===sig)return;n.innerHTML=NAV.map(([id,icon,label])=>`<button type="button" data-page="${id}" class="${id===active?'active':''}"><span class="navico">${icon}</span><span class="navtxt">${E(label)}</span></button>`).join('');n.dataset.refv6Sig=sig}
