...
(function(){
'use strict';
if(window.__BLIS_PRODUCTION_ENTRY_V43)return;window.__BLIS_PRODUCTION_ENTRY_V43=true;
const VERSION='20260906-globe-key-factors-1';
...
function renameMarketCopy(){const nav=document.querySelector('#nav [data-page="market"],#nav [data-n3-page="market"]');if(nav){const l=nav.querySelector('.navtxt')||nav.querySelector('span:last-child');if(l&&/Карта на възприятията|Интерактивна карта/i.test(l.textContent||''))l.textContent='Ключови фактори'}const market=document.getElementById('market');if(market){market.querySelectorAll('h1,h2,h3,h4,p,span,b,strong,small').forEach(el=>{if(el.children.length)return;const t=(el.textContent||'').trim();if(t==='Карта на възприятията'||t==='Интерактивна карта на възприятията')el.textContent='Ключови фактори в информационната среда'})}const a=document.getElementById('blisActiveModule'),b=document.getElementById('blisSystemDetail');if(a&&/Карта на възприятията|Интерактивна карта/i.test(a.textContent||''))a.textContent='Ключови фактори в информационната среда';if(b&&/възприят/i.test(b.textContent||''))b.textContent='Проверими източници, измервания и връзки в наблюдаваната среда'}
...
function scheduleVisualContract(){markCanonicalVisuals();window.BLISNavigator3VisualPreservationV1?.schedule?.();window.BLISNavigator3PageContractV1?.schedule?.();setTimeout(markCanonicalVisuals,70);setTimeout(markCanonicalVisuals,240);setTimeout(renameMarketCopy,500)}
