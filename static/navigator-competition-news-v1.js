/* BLIS Navigator — Competitor Intelligence News V1
   ...
   verified signals already collected for the current client. */
(function(){
'use strict';
...
const clock=t=>t?new Date(t).toLocaleTimeString('bg-BG',{hour:'2-digit',minute:'2-digit'}):'—';
let apiRows=[],apiClient='',apiUpdated=0,apiBusy=false,apiError=false,syncTimer=0,renderTimer=0,retryTimer=0,lastRenderSignature='',observer=null,observedRoot=null;
function slug(){try{return window.BLISClientUIV3?.current?.()||new URLSearchParams(location.search).get('client')||document.body?.dataset?.client||window.BLIS_INITIAL_CLIENT||''}catch(_){return document.body?.dataset?.client||window.BLIS_INITIAL_CLIENT||''}}
function signalKey(s){return String(s?.fingerprint||s?.id||url(s)||text(s)).toLowerCase()}
...
function signals(){const c=slug();if(apiClient!==c)return[];const seen=new Set();return apiRows.filter(s=>String(s?.client||'').toLowerCase()===c&&String(s?.scope||'').toLowerCase()==='competitor'&&/^https?:\/\//i.test(url(s))&&src(s)&&text(s)).filter(s=>{const k=signalKey(s);if(!k||seen.has(k))return false;seen.add(k);return true})}
function competitors(){let rows=(String(D().slug||'')===slug()?A(D().competitors):[]).map(x=>String(typeof x==='string'?x:(x?.name||x?.label||x?.brand||'')).trim()).filter(Boolean);for(const s of apiClient===slug()?apiRows:[]){if(String(s?.client||'').toLowerCase()===slug()&&String(s?.scope||'').toLowerCase()==='competitor'&&String(s?.brand||'').trim())rows.push(String(s.brand).trim())}if(!rows.length){rows=[...document.querySelectorAll('#competition .cmpv11-row,#competition [data-competitor],#competition .competitor-name')].map(x=>(x.dataset?.competitor||x.textContent||'').replace(/\s+/g,' ').trim()).filter(x=>x&&x.length<90)}const own=String(D().name||'').toLowerCase();const seen=new Set();return rows.filter(x=>{const k=x.toLowerCase();if(!k||k===own||seen.has(k))return false;seen.add(k);return true}).slice(0,16)}
function matchCompetitor(s,names){const explicit=String(s?.brand||s?.competitor||s?.competitor_name||'').trim();const scope=String(s?.scope||'').toLowerCase(),topic=String(s?.topic||'').toLowerCase();if(scope==='competitor'&&explicit)return explicit;const hay=(' '+text(s)+' '+desc(s)+' '+explicit+' ').toLowerCase();const hit=names.find(n=>hay.includes(n.toLowerCase()));if(hit)return hit;if(/compet|конкур/.test(scope+' '+topic))return explicit||'Конкурентен сигнал';return''}
function compSignals(){const names=competitors(),cut=Date.now()-period()*864e5;return signals().map(s=>({s,name:matchCompetitor(s,names),time:ts(s)})).filter(x=>x.name&&x.time>=cut&&x.time<=Date.now()+864e5).sort((a,b)=>b.time-a.time)}
async function syncAPI(force=false){const c=slug();if(!c||c==='kub'||apiBusy)return;if(!force&&c===apiClient&&Date.now()-apiUpdated<45000)return;apiBusy=true;try{const r=await fetch(`/api/public/mentions?client=${encodeURIComponent(c)}&scope=competitor&limit=300&_=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)throw new Error('signal api '+r.status);const j=await r.json();const rows=A(j?.signals).filter(s=>String(s?.client||'').toLowerCase()===c&&String(s?.scope||'').toLowerCase()==='competitor');if(slug()!==c)return;apiRows=monotonicMerge(apiClient===c?apiRows:[],rows);apiClient=c;apiUpdated=Date.now();apiError=false;clearTimeout(retryTimer);schedule(0)}catch(e){if(slug()===c){apiError=true;schedule(0);clearTimeout(retryTimer);retryTimer=setTimeout(()=>syncAPI(true),10000)}console.warn('Competitor news sync',e)}finally{apiBusy=false;if(slug()!==c)resync(true)}}
function cleanMarket(){const market=document.getElementById('market');if(!market)return;market.querySelectorAll('#civ2-environment,.civ2').forEach(el=>{const t=(el.innerText||'').replace(/\s+/g,' ').trim();if(el.id==='civ2-environment'||/Как се променя средата|Развития в информационната среда|Официалният сайт и електронният магазин са достъпни|LinkedIn аудитория/i.test(t))el.setAttribute('data-compnews-remove','1')});market.querySelectorAll('section,div').forEach(el=>{const h=(el.querySelector(':scope > h2,:scope > h3,:scope > header h2,:scope > header h3')?.textContent||'').trim();const t=(el.innerText||'').replace(/\s+/g,' ').trim();if(/^Пазарен анализ$/i.test(h)&&/Няма отделен аналитичен сигнал за този модул/i.test(t))el.setAttribute('data-compnews-remove','1')})}
...
#market #civ2-environment,[data-compnews-remove="1"]{display:none!important}
...
#competition .compnews-status.live:before{content:'';display:inline-block;width:6px;height:6px;margin-right:6px;border-radius:50%;background:#21a76e;box-shadow:0 0 0 4px rgba(33,167,110,.10);vertical-align:middle}
...
function ticker(rows){if(!rows.length)return`<div class="compnews-empty" style="margin:0;border:0;border-radius:0;background:#f9fbfd">Няма нова потвърдена конкурентна публикация в наличния поток за избрания период.</div>`;const item=x=>{const s=x.s,u=url(s),tag=x.name||'Конкурент',title=text(s),meta=src(s)||dt(x.time);const inner=`<strong>${E(tag)}</strong><span>${E(title)}</span>${meta?`<small>${E(meta)}</small>`:''}`;return u&&/^https?:\/\//i.test(u)?`<a class="compnews-item" href="${E(u)}" target="_blank" rel="noopener">${inner}</a>`:`<div class="compnews-item">${inner}</div>`};const base=rows.slice(0,14).map(item).join('');return`<div class="compnews-ticker"><div class="compnews-track">${base}${base}</div></div>`}
function activity(rows,names){const by=new Map(names.map(n=>[n,[]]));for(const x of rows){if(!by.has(x.name))by.set(x.name,[]);by.get(x.name).push(x)}const cards=[...by.entries()].sort((a,b)=>b[1].length-a[1].length).slice(0,10).map(([name,rs])=>{const latest=rs[0];return`<div class="compnews-brand"><b>${E(name)}</b><strong>${rs.length}</strong><small>${rs.length?`сигнала / публикации${latest?.time?` · последно ${E(dt(latest.time))}`:''}`:'без нов потвърден сигнал'}</small></div>`}).join('');return cards||`<div class="compnews-empty">Няма конфигуриран конкурентен списък за този клиент.</div>`}
function latest(rows){if(!rows.length)return`<div class="compnews-empty">Няма проверено конкурентно споменаване, публикувано през избрания период. Архивните публикации не се броят като нови.</div>`;return rows.slice(0,9).map(x=>{const s=x.s,u=url(s),d=desc(s),meta=[x.name,src(s),dt(x.time)].filter(Boolean);return`<div class="compnews-card"><div><b>${E(text(s))}</b>${d&&d!==text(s)?`<p>${E(d.slice(0,220))}</p>`:''}<div class="compnews-meta">${meta.map(m=>`<span>${E(m)}</span>`).join('')}</div></div>${u&&/^https?:\/\//i.test(u)?`<a href="${E(u)}" target="_blank" rel="noopener">ИЗТОЧНИК ↗</a>`:''}</div>`}).join('')}
function render(){css();cleanMarket();const host=document.getElementById('competitionBody');if(!host)return;const rows=compSignals(),names=competitors(),sig=[slug(),period(),apiClient,apiUpdated>0,rows.map(x=>signalKey(x.s)).join('|'),names.join('|'),apiError].join('::');if(sig===lastRenderSignature&&host.querySelector('#compnews-v1'))return;lastRenderSignature=sig;document.getElementById('compnews-v1')?.remove();const status=apiError?'Временно няма връзка · показани са последните записи':apiClient===slug()&&apiUpdated?`${rows.length} споменавания · проверка на 60 сек.`:`Проверка на конкурентния поток · ${names.length} конкурента`;const html=`<section id="compnews-v1" class="compnews-v1"><div class="compnews-head"><div><span class="compnews-k">COMPETITIVE INTELLIGENCE</span><h3>Споменавания на конкурентите</h3><p>Проверими публикации за конфигурираните конкуренти през избрания период. Всеки запис води към източника; споменаването само по себе си не означава действие на конкурента.</p></div><div class="compnews-status ${apiUpdated?'live':''}">${E(status)}</div></div>${ticker(rows)}<div class="compnews-body"><div class="compnews-panel"><h4>Активност по конкуренти</h4><p>Брой налични конкурентни сигнали за избрания период. Това не е пазарен дял.</p><div class="compnews-activity">${activity(rows,names)}</div></div><div class="compnews-panel"><h4>Последни споменавания</h4><p>Публикации за конкурентите с посочен източник и време на откриване или публикуване.</p><div class="compnews-list">${latest(rows)}</div></div></div></section>`;host.insertAdjacentHTML('afterbegin',html)}
function schedule(ms=100){clearTimeout(renderTimer);renderTimer=setTimeout(render,ms)}
function resync(force=false){clearTimeout(syncTimer);syncTimer=setTimeout(()=>syncAPI(force),80)}
function watchHost(){const root=document.getElementById('competition');if(root===observedRoot)return;observer?.disconnect();observedRoot=root;if(!root)return;observer=new MutationObserver(()=>{if(root.classList.contains('active')&&root.querySelector('#competitionBody')&&!root.querySelector('#compnews-v1'))schedule(0)});observer.observe(root,{childList:true,subtree:true})}
for(const ev of ['blis:clientdata','blis:intelligence','blis:periodchange','blis:routechange'])window.addEventListener(ev,()=>{watchHost();schedule(120);resync(ev==='blis:clientdata')});
document.addEventListener('click',e=>{if(e.target.closest?.('[data-page="competition"],[data-n3-page="competition"],#nav button,.client-option,.datebox')){schedule(180);resync(e.target.closest?.('.client-option')!=null)}},true);
function boot(){css();cleanMarket();watchHost();schedule(250);syncAPI(true);setTimeout(()=>schedule(0),900);setTimeout(cleanMarket,2200);setInterval(()=>{if(document.visibilityState!=='hidden'){watchHost();schedule(0);syncAPI(false)}},60000)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.BLISCompetitionNewsV1={render,cleanMarket,competitors,compSignals,sync:()=>syncAPI(true)};
})();
