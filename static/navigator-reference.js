/* BLIS Navigator — canonical production router v5. One owner per route + safe legacy aliases. */
(function(){
'use strict';
if(window.__BLIS_REFERENCE_V5)return;window.__BLIS_REFERENCE_V5=true;
...
function nav(){const n=document.getElementById('nav');if(!n)return;const active=document.querySelector('.page.active')?.id||'overview',sig=NAV.map(([id,icon,label])=>`${id}|${icon}|${label}|${id===active?'1':'0'}`).join(';');if(n.dataset.refv5Sig===sig)return;n.innerHTML=NAV.map(([id,icon,label])=>`<button data-page="${id}" class="${id===active?'active':''}"><span class="navico">${icon}</span><span class="navtxt">${E(label)}</span></button>`).join('');n.dataset.refv5Sig=sig;n.querySelectorAll('button[data-page]').forEach(b=>b.onclick=()=>window.refGo(b.dataset.page))}
...
function renderRoute(id){id=canonical(id);try{
 ...
 return fallback(id)}
window.refGo=function(id){id=canonical(id);id=IDS.has(id)?id:'overview';activate(id);nav();const out=renderRoute(id);window.scrollTo({top:0,behavior:'smooth'});return out};window.refGo.__blisCanonicalClientRouter=true;window.BLISRouteAlias=canonical;
