/* BLIS Navigator - Black Sea Center locked client shell support. */
(function(){
'use strict';
if(window.__BLIS_BLACK_SEA_CENTER_V2)return;window.__BLIS_BLACK_SEA_CENTER_V2=true;window.__BLIS_BLACK_SEA_CENTER_V1=true;
...
const COMPETITORS=[
 {name:'Varna Towers',kind:'Бизнес и офис комплекс',url:'https://varnatowers.bg/'},
 {name:'Business Park Varna',kind:'Офис и бизнес парк',url:'https://businessparkvarna.com/'},
 {name:'Delta Planet Mall',kind:'Търговски и градски комплекс',url:'https://deltaplanet.bg/'},
 {name:'Grand Mall Varna',kind:'Търговски комплекс',url:'https://grandmall-varna.com/'}
];
let busy=false;
...
function ensureLockedClient(){
 ...
 if(sel){sel.innerHTML=`<option value="${KEY}">${NAME}</option>`;sel.value=KEY;sel.disabled=true;sel.setAttribute('aria-label',NAME)}
 const menu=document.querySelector('.client-switch-menu');if(menu){menu.innerHTML='';menu.style.display='none'}
 const wrap=document.querySelector('.client-switch');if(wrap){wrap.classList.remove('open')}
 const btn=document.querySelector('.client-switch-button');if(btn){btn.style.pointerEvents='none';btn.setAttribute('aria-expanded','false');btn.setAttribute('aria-disabled','true')}
}
function applyLogo(){
 document.querySelectorAll('.bch3-logo').forEach(slot=>{
   ...
   if(!img){slot.querySelectorAll('img').forEach(n=>n.remove());img=document.createElement('img');img.dataset.bscLogo='1';img.alt=NAME;img.src=LOGO+'?v=20260907-bsc2';slot.appendChild(img)}
...
function renameNavigation(){
 for(const [id,label] of Object.entries(NAV)){
   document.querySelectorAll(`#nav [data-n3-page="${id}"],#nav [data-page="${id}"]`).forEach(btn=>{const n=btn.querySelector('.navtxt')||btn.querySelector('span:last-child');if(n)n.textContent=label});
 }
 const ctx=document.querySelector('.bch3-context-title');if(ctx&&NAV[activePage()])ctx.textContent=NAV[activePage()];
}
function renameKeyFactors(){
 ...
 market.querySelectorAll('h1,h2,h3,h4,p,span,b,strong,small').forEach(el=>{
   ...
   if(/^(Интерактивна карта на възприятията|Карта на възприятията|Интерактивна мрежа|3D мрежа на възприятията|Ключови фактори в информационната среда)$/i.test(t))el.textContent='Ключови фактори';
 });
 const active=document.getElementById('blisActiveModule');if(active&&/Интерактивна|Карта|Ключови фактори в информационната среда/i.test(active.textContent||''))active.textContent='Ключови фактори';
}
...
function paint(){
 if(busy||!isBSC())return;busy=true;
 try{
...
function schedule(){paint();setTimeout(paint,60);setTimeout(paint,220);setTimeout(paint,700);setTimeout(paint,1500)}
...
const mo=new MutationObserver(()=>{if(busy||!isBSC())return;requestAnimationFrame(paint)});mo.observe(document.documentElement,{childList:true,subtree:true,characterData:true});
})();
