...
(function(){
  ...
  /* One official point per calendar day: last successful snapshot wins. */
  ...
  const coverage=()=>{const c=coverageDays();const needed=days===7?Math.min(3,days*.4):days*.7;return{days:c,selected:days,enough:c>=needed,snapshots:snapshots().length,total:dailyAll().length}};
  ...
  const api={get days(){return days},snapshots,dailySeries,history,score:scoreFor,currentScore,activities,signals,competitors,coverage,rangeLabel,set(n){n=Number(n);if(!VALID.includes(n))return;days=n;localStorage.setItem('blis_period_days',String(days));syncButton();window.dispatchEvent(new CustomEvent('blis:periodchange',{detail:{days,coverage:coverage()}}));}};
