...
function english(){
  if(document.documentElement.dataset.blisLang==='en'||window.BLIS_LANGUAGE==='en')return true;
  const q=(new URLSearchParams(location.search).get('lang')||'').toLowerCase();if(q==='en')return true;if(q==='bg')return false;
  try{return (localStorage.getItem('blis.language.v1')||'').toLowerCase()==='en'}catch(_){return false}
}
...
function installDigitalIntelligenceUI(){
  ...
  const en=english(),from=en?'Signals from the brand':'Сигнали от марката',about=en?'Signals about the brand':'Сигнали за марката';
  s.textContent=`
    ...
    #social #n15Signals .n15-dir.from strong:after{content:'${from}'}
    #social #n15Signals .n15-dir.about strong:after{content:'${about}'}
...
function setNav(){
  const labels=english()?{social:'Signals',digital:'Visibility',market:'Sentiment',competition:'Competitors'}:{social:'Сигнали',digital:'Видимост',market:'Нагласи',competition:'Конкуренти'};
...
function apply(){installDigitalIntelligenceUI();installTypographySystem();installCurrentOwners();setNav();replaceText(document.getElementById('market'));replaceText(document.getElementById('competition'));window.BLISI18N?.apply?.(document)}
function schedule(){requestAnimationFrame(apply)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule,{once:true});else schedule();
window.addEventListener('blis:clientdata',schedule);window.addEventListener('blis:periodchange',schedule);window.BLISUITerminology={apply};
})();
