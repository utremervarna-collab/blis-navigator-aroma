...
if(!document.getElementById('blisPrepaintGuard')){
  const g=document.createElement('style');g.id='blisPrepaintGuard';g.textContent='.page{visibility:hidden!important}.page.active{min-height:560px!important}';document.head.appendChild(g);
}
/* The legacy competition guard can leave an empty white module after later renderers run. */
document.getElementById('blisCompetitionPaintGuard')?.remove();
/* dashboard.html still has a static V16 tag. Block that copy and load a fresh one only after the final stack. */
...
window.renderAll=function(){
  ...
  }catch(e){console.error('BLIS bridge state sync failed',e)}
};

function loadScript(id,src){return new Promise(resolve=>{const old=document.getElementById(id);if(old){resolve(old);return}const s=document.createElement('script');s.id=id;s.src=src;s.async=false;s.onload=()=>resolve(s);s.onerror=()=>resolve(s);document.head.appendChild(s)})}

let started=false;
async function startFinalStack(){
  if(started)return;started=true;
  const v='20260821-1436';
  const files=[
...
/* All legacy script tags in dashboard.html finish before load. They render only behind the prepaint guard.
   This eliminates the 1–4 second legacy-screen flash instead of repainting over it later. */
