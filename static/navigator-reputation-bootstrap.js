/* BLIS Navigator — Reputation bootstrap v32. Stable, single-pass controller. */
(function(){
'use strict';
...
function run(delay=80,force=false){setTimeout(()=>{if(active())window.BLISReputationStableV32?.run?.(force)},delay)}
function wrapRefGo(){const old=window.refGo;if(typeof old!=='function'||old.__repStable32)return false;const wrapped=function(id){const r=old.apply(this,arguments);if(id==='reputation')run(90,false);return r};wrapped.__repStable32=true;window.refGo=wrapped;return true}
function init(){
 style('data-blis-reputation-polish','/navigator-reputation-polish.css?v=20260819-reputation29');
 style('data-blis-reputation-refine30','/navigator-reputation-refine-v30.css?v=20260819-reputation31');
 script('data-blis-reputation-stable32','/navigator-reputation-stable-v32.js?v=20260819-reputation32',()=>run(70,false));
 wrapRefGo();
