...
const RULES={
 ...
 institutions:/прокурат|съд|мррб|регионалн(?:ият|ия) минист|днск|данс|полици|електроразпредел|енерго|вик|водоснабд/i,
...
function numRisk(s){
 const explicit=Number(s.risk_score||s.risk||0);
 ...
 if(sev==='low')return 40;
 const rel=Number(s.relevance||0);
 return Number.isFinite(rel)?Math.max(0,Math.min(100,rel)):0;
}
...
function applyNodes(){document.querySelectorAll('.kubam-node').forEach(n=>{n.classList.remove('kubaml-hot');n.querySelectorAll('.kubaml-live').forEach(x=>x.remove());const rows=latest.signals.filter(s=>matches(n.dataset.id,s));if(!rows.length)return;const max=Math.max(...rows.map(numRisk));if(max>=80)n.classList.add('kubaml-hot');const g=document.createElementNS('http://www.w3.org/2000/svg','g');g.setAttribute('class','kubaml-live');const c=document.createElementNS('http://www.w3.org/2000/svg','circle');c.setAttribute('cx','25');c.setAttribute('cy','-25');c.setAttribute('r','10');c.setAttribute('class','kubaml-badge');const t=document.createElementNS('http://www.w3.org/2000/svg','text');t.setAttribute('x','25');t.setAttribute('y','-22.5');t.setAttribute('text-anchor','middle');t.setAttribute('class','kubaml-badge-text');t.textContent=rows.length>99?'99+':rows.length;g.append(c,t);n.appendChild(g);});}
...
function render(){const strip=ensureStrip();if(strip){const critical=latest.signals.filter(s=>numRisk(s)>=85).length;strip.innerHTML='<span class="kubaml-dot"></span><b>LIVE OPEN WEB</b><span>'+latest.signals.length+' текущи сигнала</span><span>· '+critical+' HIGH/CRITICAL</span><span class="kubaml-right">API обновено: '+fmt(latest.updated_at)+'</span>';}applyNodes();applyNarratives();enrichDetail(currentActor());}
