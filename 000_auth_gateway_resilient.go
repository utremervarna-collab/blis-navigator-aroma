...
func navigatorGateway(w http.ResponseWriter, r *http.Request) {
	...
	if path == "/wirello" || path == "/wirello/" || path == "/wirello-master-demo.html" {
		clearSession(w, r); setPublicDemoCookie(w, r); r2 := r.Clone(r.Context()); r2.URL.Path = "/dashboard.html"; q := r2.URL.Query(); q.Set("client", "wirello"); if q.Get("page") == "" { q.Set("page", "overview") }; r2.URL.RawQuery = q.Encode(); r2.Header.Set("X-BLIS-Client-Scope", "wirello"); authProxy.ServeHTTP(w, r2); return
	}
	if isWirelloDemo(r) {
	...
	if path == "/navigator" || path == "/navigator/" {
		...
		if s, ok := sessionFromRequest(r); ok && s.Admin { http.Redirect(w, r, navigatorDashboardTarget(r), http.StatusFound); return }
