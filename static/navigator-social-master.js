/* BLIS Navigator — Социални сигнали v6. Един източник на истина, измерена история, каноничен feed. */
(function(){
  'use strict';
  ...
  const fmt=(v,d=0)=>v==null?'0':Number(v).toLocaleString('bg-BG',{maximumFractionDigits:d,minimumFractionDigits:d});
  const pct=v=>fmt(v,1)+'%';
  ...
  function styles(){
    if(document.getElementById('smV6Styles'))return;
    const s=document.createElement('style');s.id='smV6Styles';s.textContent=`
  ...
  function canonical(sources,obs){
    ...
    for(const [platform,list] of groups){
      ...
      const followers=latest(obs,keys,/followers|audience|последовател|subscriber|абонат/)?.value??null;
      const access=latest(obs,keys,/public_page_access/)?.value??((latest(obs,keys,/profile_active|reachable/)?.value||0)*100);
      const posts=latest(obs,keys,/visible_posts|recent_videos_90d|recent_public_posts|publication_count|post_count/)?.value??0;
      const engRate=latest(obs,keys,/engagement_rate|ангажираност/)?.value??null;
      const likes=latest(obs,keys,/^likes$/)?.value??0,comments=latest(obs,keys,/comments_visible/)?.value??0,shares=latest(obs,keys,/shares_visible/)?.value??0,markers=latest(obs,keys,/visible_reaction_markers/)?.value??0;
      const reactions=likes+comments+shares+markers;
      const mentions=latest(obs,keys,/mentions|mention_count|brand_mentions_visible|aroma_mentions/)?.value??0;
  ...
  function channelMetric(label,value,suffix=''){return `<span>${esc(label)}<b>${esc(fmt(value,suffix==='%'?1:0)+(suffix||''))}</b></span>`}
  ...
  function postFeeds(chs,obs){
    return `<div class="sm-card sm-network-feeds"><div class="sm-card-head"><div><h3>ПОСЛЕДНИ ПУБЛИКАЦИИ ПО КАНАЛИ</h3><p>До три последно достъпни публични публикации от всеки наблюдаван профил</p></div><span class="sm-pill">ПУБЛИЧНИ ИЗТОЧНИЦИ</span></div><div class="sm-network-feed-grid">${chs.map(ch=>{const posts=displayPosts(ch,obs),count=Number(ch.posts||0);return `<section class="sm-network-feed ${posts.length?'':'is-empty'}"><div class="sm-network-feed-head">${platformIcon(ch.platform)}<b>${esc(ch.platform)}</b><small>${posts.length?`показани ${posts.length}${count>posts.length?` от ${fmt(count)}`:''}`:`${fmt(count)} измерени`}</small></div><div class="sm-network-posts">${posts.length?posts.map(p=>`<a class="sm-network-post" ${p.url?`href="${esc(p.url)}" target="_blank" rel="noopener noreferrer"`:''}><div><p>${esc(p.text)}</p><span>${esc(dateBG(p.time))}${p.verified?' · публично потвърдено':''}</span></div>${p.url?'<em>Отвори ↗</em>':''}</a>`).join(''):`<div class="sm-network-empty">${count>0?'Има измерена публикационна активност, но чистият текст на последните постове не е публично достъпен без вход/API.':'При последната проверка не е открит публично достъпен текст на публикация.'}</div>${ch.url?`<a class="sm-network-profile" href="${esc(ch.url)}" target="_blank" rel="noopener noreferrer">Отвори профила ↗</a>`:''}`}</div></section>`}).join('')}</div></div>`;
  }
  ...
  async function render(){
    ...
    chs.forEach(ch=>{const hasContent=(feedMap.get(ch.key)||[]).length>0;ch.active=ch.access>0||ch.followers!=null||ch.posts>0||ch.reactions>0||hasContent;});
    const activeCount=chs.filter(c=>c.active).length;
    const followers=chs.map(x=>x.followers).filter(v=>v!=null),audience=followers.reduce((a,b)=>a+b,0),totalPosts=chs.reduce((a,c)=>a+(Number(c.posts)||0),0),rates=chs.map(x=>x.engRate).filter(v=>v!=null),totalReactions=chs.reduce((a,c)=>a+(Number(c.reactions)||0),0),engagement=rates.length?rates.reduce((a,b)=>a+b,0)/rates.length:null,totalMentions=chs.reduce((a,c)=>a+(Number(c.mentions)||0),0);
