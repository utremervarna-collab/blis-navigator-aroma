(() => {
  ...
  function renderLower(){
    const host=qs('#market .pm-lower');if(!host)return;const series=perceptionSeries(),activity=activitySeries(),current=series.at(-1)?.v??coreValue('perception')??composite(getD()),delta=series.length>1?series.at(-1).v-series.at(-2).v:null,activityCurrent=activity.at(-1)?.v??null,activityDelta=activity.length>1?activity.at(-1).v-activity.at(-2).v:null,rows=latestRows(),sources=new Set(rows.map(r=>sourceOf(r.x)).filter(Boolean)).size;
    const sig=JSON.stringify({series:series.map(x=>[x.t,x.v]),activity:activity.map(x=>[x.t,x.v]),current,delta,rows:rows.map(r=>[metricOf(r.x),r.x.value,sourceOf(r.x),r.t])});if(host.dataset.pmsSig===sig&&qs('.pms-bottom-card',host))return;host.dataset.pmsSig=sig;
