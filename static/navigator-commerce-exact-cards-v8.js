...
function apply(card){const id=idOf(card),src=MAP[id];if(!src)return;if(card.dataset.bc8==='1')return;const req=card.querySelector('[data-bc3-request]'),more=card.querySelector('[data-bc3-detail]');card.dataset.bc8='1';card.classList.add('bc8-exact-card');const poster=document.createElement('div');poster.className='bc8-poster';poster.innerHTML=`<img src="${src}?v=20260824-approved-light3" alt="${id}" loading="eager" decoding="async"><button type="button" class="bc8-hit bc8-hit-order" aria-label="Заяви"></button><button type="button" class="bc8-hit bc8-hit-more" aria-label="Виж повече"></button>`;poster.querySelector('.bc8-hit-order').onclick=e=>{e.preventDefault();e.stopPropagation();req?.click()};poster.querySelector('.bc8-hit-more').onclick=e=>{e.preventDefault();e.stopPropagation();more?.click()};card.prepend(poster)}
