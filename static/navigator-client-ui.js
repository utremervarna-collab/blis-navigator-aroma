...
const clients={
  ...
  'black-sea-center':{bg:'Black Sea Center',en:'Black Sea Center',typeBg:'Бизнес и търговски комплекс',typeEn:'Business & retail complex'}
};
...
const visibleOrder=()=>lockedBSC()?['black-sea-center']:defaultVisibleOrder;
...
function ensureSelect(){const sel=document.getElementById('clientSel');if(!sel)return;sel.innerHTML='';for(const k of (lockedBSC()?['black-sea-center']:Object.keys(clients))){const o=document.createElement('option');o.value=k;o.textContent=label(k);sel.appendChild(o)}if(lockedBSC())sel.disabled=true}
function ensureMenu(){const menu=document.querySelector('.client-switch-menu');if(!menu)return;const active=current();menu.innerHTML=visibleOrder().map(k=>`<button type="button" class="client-option${k===active?' active':''}" data-client-key="${k}" role="option" aria-selected="${k===active?'true':'false'}"><span><b>${esc(label(k))}</b><small>${esc(type(k))}</small></span><span class="client-option-check" aria-hidden="true">${k===active?'✓':''}</span></button>`).join('');if(lockedBSC()){menu.style.display='none';const btn=document.querySelector('.client-switch-button');if(btn){btn.style.pointerEvents='none';btn.setAttribute('aria-expanded','false')}}}
...
function init(){if(current()==='kub'){location.replace('/kub-crisis.html');return}ensureSelect();ensureMenu();const k=current(),sel=document.getElementById('clientSel');if(sel)sel.value=k;paint(k);const wrap=document.querySelector('.client-switch');if(wrap){wrap.style.position='relative';wrap.style.zIndex='200'}const menu=document.querySelector('.client-switch-menu');if(menu)menu.style.zIndex='1000';if(!wired){wired=true;window.__BLIS_CLIENT_SWITCH_WIRED_V5=true;document.addEventListener('click',click,true);document.addEventListener('keydown',e=>{if(e.key==='Escape')close()});window.addEventListener('popstate',()=>{ensureMenu();paint(current())});['blis:clientdata','blis:production-ready'].forEach(ev=>window.addEventListener(ev,()=>{ensureMenu();paint(current())}))}}
window.BLISClientUIV5={select,paint,current,visibleClients:visibleOrder(),clients};window.BLISClientUIV4=window.BLISClientUIV5;window.BLISClientUIV3=window.BLISClientUIV5;if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();