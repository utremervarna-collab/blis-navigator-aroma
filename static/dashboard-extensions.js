...
(function(){
  const extensionPages=[
    ...
    ['market','↗','Пазарни сигнали'],
    ['competition','⚑','Конкурентна позиция'],
    ['timeline','◷','Timeline'],
  ...
  window.nav=function(){
    ...
    if(!navEl) return;
    navEl.innerHTML=extensionPages.map((p,i)=>`<button data-page="${p[0]}" class="${i?'':'active'}"><span class="navico navsym">${icons[p[0]]||p[1]}</span><span class="navtxt">${p[2]}</span></button>`).join('');
    document.querySelectorAll('#nav button').forEach(b=>b.onclick=()=>window.go(b.dataset.page));
  };
  const oldGo=window.go;
  window.go=function(id){
    if(typeof oldGo==='function') oldGo(id);
    if(id==='timeline') window.renderTimeline();
    if(id==='settings') window.renderSettings();
  };
  ...
  function timelineEvents(){
    let out=[];
    (window.D?.signals||[]).forEach(s=>out.push({time:s.time||s.created_at||new Date().toISOString(),title:s.title||s.label||'Аналитичен сигнал',text:s.description||s.detail||'Промяна в наблюдаваната среда.',type:/risk|негатив|оплак/i.test((s.title||'')+' '+(s.description||''))?'risk':/конкур/i.test((s.title||'')+' '+(s.description||''))?'competitive':'positive'}));
    (window.A||[]).slice(0,16).forEach(x=>out.push({time:x.time||x.observed_at||new Date().toISOString(),title:typeof window.metricName==='function'?window.metricName(x.metric):String(x.metric||'Показател'),text:`${typeof window.sourceName==='function'?window.sourceName(x.source):(x.source||'Източник')} · ${typeof window.activityValue==='function'?window.activityValue(x):(x.value??'—')}`,type:/rating|review/i.test(x.metric||'')?'reputation':'data'}));
    (window.H||[]).slice(-8).forEach(s=>out.push({time:s.created_at||new Date().toISOString(),title:'BLIS snapshot',text:`Записана сравнима историческа стойност${s.payload?.blis_index!=null?`: BLIS ${s.payload.blis_index}`:''}.`,type:'data'}));
    return out.sort((a,b)=>new Date(b.time)-new Date(a.time)).slice(0,30);
  }
  function safeHead(t,p){return typeof window.head==='function'?window.head(t,p):`<div class="genericHead"><div><h2>${t}</h2><p>${p}</p></div></div>`}
  function safeWatch(bg,c,ico,t,p){return typeof window.watchCard==='function'?window.watchCard(bg,c,ico,t,p):`<div class="watchCard"><div class="watchIcon" style="background:${bg};color:${c}">${ico}</div><b>${t}</b><p>${p}</p></div>`}
  window.renderTimeline=function(){
    const target=document.getElementById('timelineBody'); if(!target) return;
    let events=timelineEvents(), A=window.A||[], H=window.H||[], S=window.S||[];
    target.innerHTML=safeHead('Intelligence Timeline','Хронология на промените: измервания, сигнали, конкурентни проявления, репутационни движения и исторически snapshots.')+`<div class="grid cols12"><div class="card moduleCard c3"><div class="cardLabel">Събития</div><div class="moduleVal">${events.length}</div><div class="subtle">Събрани от текущите измервания и сигнали.</div></div><div class="card moduleCard c3"><div class="cardLabel">Измервания</div><div class="moduleVal">${A.length}</div><div class="subtle">Реални наблюдавани стойности.</div></div><div class="card moduleCard c3"><div class="cardLabel">Snapshots</div><div class="moduleVal">${H.length}</div><div class="subtle">Исторически точки за сравнение.</div></div><div class="card moduleCard c3"><div class="cardLabel">Източници</div><div class="moduleVal">${S.length}</div><div class="subtle">Информационна база на профила.</div></div><div class="card timelinePanel c8"><div class="sectionTitle">Какво се промени</div><div class="timelineList">${events.length?events.map(e=>{let t=eventTone(e.type);return `<div class="timelineEvent"><div class="timelineDot" style="background:${t[0]};color:${t[1]}">${t[2]}</div><div><div class="timelineTime">${new Date(e.time).toLocaleString('bg-BG')}</div><b>${e.title}</b><p>${e.text}</p></div></div>`}).join(''):'<div class="scan">Timeline ще се попълва автоматично при нови измервания и потвърдени сигнали.</div>'}</div></div><div class="card moduleCard c4"><div class="sectionTitle">Как се използва Timeline</div><div class="watch" style="grid-template-columns:1fr">${safeWatch('#eef5ff','#0f5fe9','◷','Последователност','Показва кога точно е възникнала промяната и как се е развила след това.')}${safeWatch('#fff8e8','#b66b00','⚑','Контекст','Свързва конкурентни, пазарни и комуникационни движения в една хронология.')}${safeWatch('#fff0f7','#c52f78','♡','Репутация','Показва кога се появява повтаряща се тема или промяна в оценките.')}${safeWatch('#effaf2','#16813a','↗','Проверка на ефект','Позволява да се сравни период преди и след значимо действие или външно събитие.')}</div></div></div><div class="pulse"><strong>BLIS Timeline</strong><span>Не е архив на числа, а последователна история на значимите промени в средата на клиента.</span></div>`;
  };
  ...
  window.renderSettings=function(){
    ...
    document.body.classList.toggle('compact-dashboard',get('compact',false));
    document.body.classList.toggle('no-motion',!get('motion',true));
  };
  const oldRenderAll=window.renderAll;
  if(typeof oldRenderAll==='function') window.renderAll=function(){oldRenderAll();window.renderTimeline();window.renderSettings();};
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>setTimeout(window.nav,0)); else setTimeout(window.nav,0);
})();