/* BLIS Navigator — Reputation refinement v31 naming. */
(function(){
'use strict';
...
function setIdentity(client){const name=NAMES[client]||'Клиент';const em=document.querySelector('#reputationBody .rp-emblem');if(em){em.dataset.client=client;const mono=em.querySelector('span'),b=em.querySelector('b');if(mono){mono.textContent='';mono.style.display='none'}if(b){b.textContent=name;b.style.display='block'}}const sub=document.querySelector('#reputationBody .rp-head p');if(sub)sub.textContent=`Публичното възприятие и репутационната среда на ${name} — само от измерими и проверими данни.`;}
...
function renamePanels(change,topicCount){const dyn=document.querySelector('#reputationBody .rp-dynamics header h3');if(dyn)dyn.textContent='Репутационни изменения';const dynP=document.querySelector('#reputationBody .rp-dynamics header p');if(dynP)dynP.textContent='Реални изменения на репутационния индекс в измерената история';const dynV=document.querySelector('#reputationBody .rp-dynamics header>b');if(dynV)dynV.textContent=change==null?'—':`${change>0?'+':''}${F(change)} т.`;
const topics=document.querySelector('#rpTopics header h3');if(topics)topics.textContent='Контекст';const topicsP=document.querySelector('#rpTopics header p');if(topicsP)topicsP.textContent=`${topicCount} измерени теми и сигнали от публичната среда`;const link=document.querySelector('#rpTopics .rp-panel-link');if(link)link.childNodes[0].nodeValue='Целият контекст ';}
...
const balance=findKpi('Репутационен баланс')||findKpi('Източниково покритие')||findKpi('Активни източници');setKpi(balance,'Активни източници',sourceCount,' източника','Активни наблюдавани източници в клиентския профил');
...
document.getElementById('reputationBody')?.setAttribute('data-rp-refined','31');
}finally{busy=false}}
