...
function loadScript(id,src){if(document.getElementById(id)||[...document.scripts].some(s=>s.src&&s.src.includes(src.split('?')[0])))return;const s=document.createElement('script');s.id=id;s.src=src;s.async=false;document.head.appendChild(s)}
function loadStyle(id,href){if(document.getElementById(id)||[...document.styleSheets].some(s=>{try{return s.href&&s.href.includes(href.split('?')[0])}catch(_){return false}}))return;const l=document.createElement('link');l.id=id;l.rel='stylesheet';l.href=href;document.head.appendChild(l)}
function loadOwners(){
  const v='20260824-core5';
  ...
  loadScript('blisOverviewSafeScript','/navigator-overview-safe-v3.js?v='+v);
}
...
function boot(){loadOwners();syncChrome();releasePrepaint();window.addEventListener('blis:clientdata',()=>syncChrome())}
