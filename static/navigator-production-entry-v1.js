...
(function(){
'use strict';
if(window.__BLIS_PRODUCTION_ENTRY_V44)return;window.__BLIS_PRODUCTION_ENTRY_V44=true;
const VERSION='20260907-loading-guard-1';
...
function renameMarketCopy(){const nav=document.querySelector('#nav [data-page="market"],#nav [data-n3-page="market"]');if(nav){const l=nav.querySelector('.navtxt')||nav.querySelector('span:last-child');if(l)l.textContent='Ключови фактори'}const market=document.getElementById('market');if(market){market.querySelectorAll('h1,h2,h3,h4,p,span,b,strong,small').forEach(el=>{if(el.children.length)return;const t=(el.textContent||'').trim();if(/Карта на възприятията|Интерактивна карта на възприятията|Ключови фактори в информационната среда/i.test(t))el.textContent='Ключови фактори'})}const a=document.getElementById('blisActiveModule'),b=document.getElementById('blisSystemDetail');if(a&&/Карта на възприятията|Интерактивна карта|Ключови фактори в информационната среда/i.test(a.textContent||''))a.textContent='Ключови фактори';if(b&&/възприят/i.test(b.textContent||''))b.textContent='Проверими източници, измервания и връзки в наблюдаваната среда'}
