...
function renderSoon(delay=40){setTimeout(()=>{try{if(document.getElementById('reputation')?.classList.contains('active')){window.BLISReputation?.render?.();setTimeout(polishTruth,25);hydrateSoon(70);if(!window.BLISReputation?._refreshed){window.BLISReputation._refreshed=true;setTimeout(()=>window.BLISReputation?.refresh?.(),80)}}}catch(e){console.error('BLIS Reputation render failed',e)}},delay)}
...
function clientRefresh(){try{window.BLISReputation._refreshed=false;window.BLISReputation?.onClient?.();renderSoon(80);renderSoon(260);hydrateSoon(340)}catch(e){}}
...
function init(){
  ...
  let n=0;const t=setInterval(()=>{n++;wrapRefGo();const page=document.getElementById('reputation');if(page?.classList.contains('active')&&!page.querySelector('.rp-screen'))renderSoon(20);if(page?.classList.contains('active')){polishTruth();window.BLISReputationRealDataV29?.sync?.()}if(n>45)clearInterval(t)},160);
  ...
  const root=document.getElementById('reputationBody');if(root)new MutationObserver(()=>{if(document.getElementById('reputation')?.classList.contains('active')){setTimeout(polishTruth,0);hydrateSoon(100)}}).observe(root,{childList:true,subtree:true});
}
