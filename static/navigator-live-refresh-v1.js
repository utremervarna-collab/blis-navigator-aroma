/* BLIS Navigator — visible mention streams v5.
   Lightweight, event-driven chronology for Monitoring and Competition. */
(function(){
'use strict';
if(window.__BLIS_LIVE_REFRESH_V5)return;
...
function renderCompetition(){
  ...
  const sig=[c,days(),st.updated,st.error,all.map(key).join('|')].join('::');if(p.dataset.signature===sig)return;p.dataset.signature=sig;
  p.innerHTML=`<div class="blis-ms-head"><div><h3>Хронология на конкурентните споменавания</h3><p>Проверими публикации за конфигурираните конкуренти на текущия бранд.</p></div>${status(st)}</div>${ticker(all,re)}<div class="blis-ms-count" style="padding-top:13px"><b>${re.length}</b> в избрания период · ${all.length} общо в наличната хронология</div><div class="blis-ms-timeline">${timeline(all,'competitor')}</div>`
}
...
window.BLISLiveRefresh={refreshMentions,renderStreams,interval:MENTION_MS,mentionInterval:MENTION_MS,version:'5.0-balanced-competitor-ticker'};
