...
func navigatorDashboardTarget(r *http.Request) string {
	...
	if !validNavigatorClient(slug) { if c, err := r.Cookie(adminClientCookieName); err == nil && validNavigatorClient(c.Value) { slug = c.Value } }
	if !validNavigatorClient(slug) { slug = "aroma" }
	q := url.Values{}; q.Set("client", slug); q.Set("page", canonicalNavigatorPage(r.URL.Query().Get("page"))); if strings.EqualFold(strings.TrimSpace(r.URL.Query().Get("lang")), "en") { q.Set("lang", "en") }
...
func init() {
	...
	internal := strconv.Itoa(p + 1); os.Setenv("BLIS_NAVIGATOR_GATEWAY_BOOTSTRAPPED", "1"); os.Setenv("BLIS_AUTH_PROXY_DISABLED", "1"); authExternalPort = external; authInternalPort = internal; os.Setenv("PORT", internal)
