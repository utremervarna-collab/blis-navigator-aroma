/* BLIS Navigator — canonical production router v4. One owner per route. */
(function(){
'use strict';
if(window.__BLIS_REFERENCE_V4)return;window.__BLIS_REFERENCE_V4=true;
...
function nav(){const n=document.getElementById('nav');if(!n)return;const active=document.querySelector('.page.active')?.id||'overview',sig=NAV.map(([id,icon,label])=>`${id}|${icon}|${label}|${id===active?'1':'0'}`).join(';');if(n.dataset.refv4Sig===sig)return;n.innerHTML=NAV.map(([id,icon,label])=>`<button data-page="${id}" class="${id===active?'active':''}"><span class="navico">${icon}</span><span class="navtxt">${E(label)}</span></button>`).join('');n.dataset.refv4Sig=sig;n.querySelectorAll('button[data-page]').forEach(b=>b.onclick=()=>window.refGo(b.dataset.page))}
function activate(id){ensurePages();if(!IDS.has(id))id='overview';document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===id));document.querySelectorAll('#nav button[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===id));try{const u=new URL(location.href);u.searchParams.set('client',new URLSearchParams(location.search).get('client')||window.BLIS_INITIAL_CLIENT||document.body?.dataset?.client||'aroma');u.searchParams.set('page',id);u.hash='';history.replaceState({page:id},'',u.pathname+u.search)}catch(_){}}
...
function renderRoute(id){try{
 ...
 return fallback(id)}
window.refGo=function(id){id=IDS.has(id)?id:'overview';activate(id);nav();const out=renderRoute(id);window.scrollTo({top:0,behavior:'smooth'});return out};window.refGo.__blisCanonicalClientRouter=true;
