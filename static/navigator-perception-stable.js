(() => {
  ...
  function ageClock(t){if(!t)return'--:--:--';let s=Math.max(0,Math.floor((Date.now()-t)/1000));const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60),sec=s%60;const hh=String(h).padStart(2,'0'),mm=String(m).padStart(2,'0'),ss=String(sec).padStart(2,'0');return d?`${d}д ${hh}:${mm}:${ss}`:`${hh}:${mm}:${ss}`}
  ...
  function activitySeries(){
    const cut=Date.now()-periodDays()*864e5,by=new Map();
    getA().forEach(x=>{const t=timeOf(x);if(!t||t<cut||!isClientMetric(x))return;const day=new Date(t).toISOString().slice(0,10);by.set(day,(by.get(day)||0)+1)});
    return[...by.entries()].sort((a,b)=>a[0].localeCompare(b[0])).map(([day,v])=>({t:new Date(day+'T12:00:00').getTime(),v}))
  }
  ...
  function smoothPath(points){
    ...
    for(let i=0;i<points.length-1;i++){
      const p0=points[i],p1=points[i+1],pm=points[i-1]||p0,pn=points[i+2]||p1;
      let c1y=p0.y+(p1.y-pm.y)/6,c2y=p1.y-(pn.y-p0.y)/6;
      const lo=Math.min(p0.y,p1.y),hi=Math.max(p0.y,p1.y);c1y=Math.max(lo,Math.min(hi,c1y));c2y=Math.max(lo,Math.min(hi,c2y));
      const c1x=p0.x+(p1.x-pm.x)/6,c2x=p1.x-(pn.x-p0.x)/6;
  ...
  function chart(series){
    ...
    const w=640,h=150,l=10,r=10,t=10,b=22,vals=series.map(x=>x.v),mn=Math.min(...vals),mx=Math.max(...vals),pad=Math.max((mx-mn)*.24,1),min=mn-pad,max=mx+pad,span=max-min||1,minT=series[0].t,maxT=series.at(-1).t,tr=Math.max(1,maxT-minT),X=x=>l+(w-l-r)*(x.t-minT)/tr,Y=v=>t+(h-t-b)*(1-(v-min)/span),pts=series.map(x=>({x:X(x),y:Y(x.v),raw:x})),line=smoothPath(pts),area=`${line} L${pts.at(-1).x.toFixed(1)} ${h-b} L${pts[0].x.toFixed(1)} ${h-b} Z`;
    return`<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"><defs><linearGradient id="pmsStableArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#1766e8" stop-opacity=".25"/><stop offset=".58" stop-color="#1766e8" stop-opacity=".08"/><stop offset="1" stop-color="#1766e8" stop-opacity="0"/></linearGradient></defs><line class="grid" x1="${l}" y1="${t+28}" x2="${w-r}" y2="${t+28}"/><line class="grid" x1="${l}" y1="${t+72}" x2="${w-r}" y2="${t+72}"/><line class="grid" x1="${l}" y1="${h-b}" x2="${w-r}" y2="${h-b}"/><path class="area" d="${area}"/><path class="line" d="${line}"/>${pts.map(p=>`<circle cx="${p.x}" cy="${p.y}" r="3"><title>${new Date(p.raw.t).toLocaleDateString('bg-BG')} · ${fmt(p.raw.v,1)}</title></circle>`).join('')}<text x="${l}" y="${h-3}">${new Date(minT).toLocaleDateString('bg-BG',{day:'2-digit',month:'2-digit'})}</text><text x="${w-r}" y="${h-3}" text-anchor="end">${new Date(maxT).toLocaleDateString('bg-BG',{day:'2-digit',month:'2-digit'})}</text></svg>`
  }
  function miniChart(series){
    if(series.length<2)return'';
    const w=640,h=64,l=10,r=10,t=7,b=8,vals=series.map(x=>x.v),mn=Math.min(...vals),mx=Math.max(...vals),pad=Math.max((mx-mn)*.18,.75),min=Math.max(0,mn-pad),max=mx+pad,span=max-min||1,minT=series[0].t,maxT=series.at(-1).t,tr=Math.max(1,maxT-minT),X=x=>l+(w-l-r)*(x.t-minT)/tr,Y=v=>t+(h-t-b)*(1-(v-min)/span),pts=series.map(x=>({x:X(x),y:Y(x.v),raw:x})),line=smoothPath(pts),area=`${line} L${pts.at(-1).x.toFixed(1)} ${h-b} L${pts[0].x.toFixed(1)} ${h-b} Z`;
    return`<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"><defs><linearGradient id="pmsActivityArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#12a66a" stop-opacity=".18"/><stop offset="1" stop-color="#12a66a" stop-opacity="0"/></linearGradient></defs><line class="mini-grid" x1="${l}" y1="${h-b}" x2="${w-r}" y2="${h-b}"/><path class="mini-area" d="${area}"/><path class="mini-line" d="${line}"/>${pts.map(p=>`<circle cx="${p.x}" cy="${p.y}" r="2.1"><title>${new Date(p.raw.t).toLocaleDateString('bg-BG')} · ${p.raw.v} значими измервания</title></circle>`).join('')}</svg>`
  }
  ...
  function renderLower(){
    const host=qs('#market .pm-lower');if(!host)return;const series=perceptionSeries(),activity=activitySeries(),current=series.at(-1)?.v??coreValue('perception')??composite(getD()),delta=series.length>1?series.at(-1).v-series.at(-2).v:null,activityCurrent=activity.at(-1)?.v??null,activityDelta=activity.length>1?activity.at(-1).v-activity.at(-2).v:null,rows=latestRows(),sources=new Set(rows.map(r=>sourceOf(r.x)).filter(Boolean)).size,latest=latestUpdate();
    const sig=JSON.stringify({series:series.map(x=>[x.t,x.v]),activity:activity.map(x=>[x.t,x.v]),current,delta,latest,rows:rows.map(r=>[metricOf(r.x),r.x.value,sourceOf(r.x),r.t])});if(host.dataset.pmsSig===sig&&qs('.pms-bottom-card',host))return;host.dataset.pmsSig=sig;
    const mini=activity.length>1?`<div class="pms-mini-trend"><div class="pms-mini-head"><div><b>Активност на сигналите</b><span>значими измервания по дни</span></div><div class="pms-mini-value"><strong>${activityCurrent}</strong>${activityDelta==null?'':`<em class="${activityDelta>0?'up':activityDelta<0?'down':''}">${activityDelta>0?'↑':activityDelta<0?'↓':'→'} ${Math.abs(activityDelta)}</em>`}</div></div><div class="pms-mini-chart">${miniChart(activity)}</div></div>`:'';
    const live=`<div class="pms-live-side"><span class="pms-bottom-live"><i></i>актуални данни</span>${latest?`<span class="pms-live-clock"><small>от последен сигнал</small><b data-pms-live-clock="${latest}">${ageClock(latest)}</b></span>`:''}</div>`;
    host.innerHTML=`<section class="pm-card pms-bottom-card pms-trend-card"><div class="pms-bottom-head"><div><h3>Как се променя възприятието</h3><p>Общата посока през избрания период</p></div>${live}</div><div class="pms-trend-top"><div class="pms-trend-number"><strong>${current==null?'—':fmt(current,1)}</strong><span>/100</span></div>${delta==null?'':`<div class="pms-trend-change ${delta>0?'up':delta<0?'down':''}">${delta>0?'↑':delta<0?'↓':'→'} ${fmt(Math.abs(delta),1)} т.</div>`}</div><div class="pms-chart">${chart(series)}</div>${mini}</section><section class="pm-card pms-bottom-card"><div class="pms-bottom-head"><div><h3>Последно отчетени показатели</h3><p>Най-новите измерими данни с значение за бранда</p></div><span class="pms-bottom-live"><i></i>АКТУАЛНО</span></div><div class="pms-stream">${rows.length?rows.map(({x,t})=>`<div class="pms-stream-row"><i class="pms-stream-dot"></i><div class="pms-stream-copy"><b>${esc(metricName(metricOf(x)))}</b><small>${esc(sourceName(sourceOf(x)))}</small></div><div class="pms-stream-value"><b>${esc(valueText(x))}</b><small data-pms-time="${t}">${esc(relTime(t))}</small></div></div>`).join(''):'<div class="pms-chart-empty">Няма нови клиентски измервания за избрания период.</div>'}</div>${rows.length?`<div class="pms-stream-foot"><span>${rows.length} последни показателя</span><span>${sources} източника</span></div>`:''}</section>`
  }
  ...
  function updateLiveClock(){qsa('#market [data-pms-live-clock]').forEach(el=>{const t=Number(el.dataset.pmsLiveClock);if(t)el.textContent=ageClock(t)})}
  function initialBoot(){[80,260,650,1100,1700].forEach(ms=>setTimeout(()=>{if(marketActive())applyAll()},ms));clearInterval(clockTimer);clockTimer=setInterval(()=>{if(marketActive()){updateTimes();updateLiveClock()}},1000)}
