/* BLIS Navigator — canonical client switcher.
   Owner sessions see the full real-client catalogue except Wirello.
...
const ownerVisibleOrder=['kub','mollox','aroma','bolyarka','astor-garden','everbet','varna-towers','black-sea-center'];
...
function selectOrder(){const order=visibleOrder();const active=current();if(valid(active)&&active!=='wirello'&&!order.includes(active))order.unshift(active);return order}
