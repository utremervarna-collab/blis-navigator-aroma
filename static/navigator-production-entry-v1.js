/* BLIS Navigator 3.0 — canonical production entrypoint.
   Bulgarian client interface with simplified five-page analytical architecture and two resources. */
(function(){
'use strict';
if(window.__BLIS_PRODUCTION_ENTRY_V41)return;window.__BLIS_PRODUCTION_ENTRY_V41=true;
const VERSION='20260901-navigator3-simplified-5plus2-2';
...
function scheduleVisualContract(){markCanonicalVisuals();setTimeout(markCanonicalVisuals,70);setTimeout(markCanonicalVisuals,240)}
async function boot(){
 forceBulgarianEarly();
 ...
 await safe('/navigator-3-architecture-v1.js');await safe('/navigator-3-stability-v1.js');
 ...
 document.documentElement.dataset.navigatorUi='navigator3-simplified-five-plus-two';
 window.dispatchEvent(new CustomEvent('blis:production-ready',{detail:{client:initialClient,page:window.BLISRouteAlias?.(new URLSearchParams(location.search).get('page')||'overview')||'overview',version:VERSION}}));
 setTimeout(()=>{forceBulgarianEarly();window.BLISClientPerspectiveClassifierV1?.repaint?.();window.BLISClientBrandingV5?.paint?.();window.BLISOverviewMarkerFixV1?.align?.();window.BLISColorSystemV1?.decorate?.();window.BLISNoPageNumbersV1?.clean?.();window.BLISLanguageCleanupV1?.clean?.();window.BLISNavigator3ClientClarity?.decorate?.();window.BLISNavigator3ClientProofV1?.decorate?.();window.BLISNavigator3ArchitectureV1?.render?.();window.BLISNavigator3StabilityV1?.schedule?.();scheduleVisualContract()},120);
}
