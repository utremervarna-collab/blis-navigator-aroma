...
function nav(){const n=document.getElementById('nav');if(!n)return;const active=document.querySelector('.page.active')?.id||'overview',sig=NAV.map(([id,icon,label])=>`${id}|${icon}|${label}|${id===active?'1':'0'}|${currentClient()}`).join(';');if(n.dataset.refv8Sig===sig)return;n.innerHTML=NAV.map(([id,icon,label])=>`<a href="${E(routeHref(id))}" data-page="${id}" class="${id===active?'active':''}"><span class="navico">${icon}</span><span class="navtxt">${E(label)}</span></a>`).join('');n.dataset.refv8Sig=sig}
...
function bindProgressiveNav(){if(window.__BLIS_NAV_PROGRESSIVE_V1)return;window.__BLIS_NAV_PROGRESSIVE_V1=true;document.addEventListener('click',function(e){if(e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;const a=e.target.closest?.('#nav a[data-page]');if(!a||typeof window.refGo!=='function')return;e.preventDefault();e.stopPropagation();window.refGo(a.dataset.page)},true)}
function boot(){ensurePages();syncGlobals();nav();bindProgressiveNav();const q=canonical(new URLSearchParams(location.search).get('page')),active=canonical(document.querySelector('.page.active')?.id),id=IDS.has(q)?q:(IDS.has(active)?active:'overview');activate(id);nav();renderRoute(id);window.addEventListener('blis:clientdata',()=>requestAnimationFrame(()=>{syncGlobals();nav();window.BLISCanonicalRenderActive()}));window.addEventListener('blis:periodchange',()=>requestAnimationFrame(()=>window.BLISCanonicalRenderActive()))}
