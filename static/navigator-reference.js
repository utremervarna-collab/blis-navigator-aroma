/* BLIS Navigator — canonical 12-page production router v2. */
(function(){
'use strict';
if(window.__BLIS_REFERENCE_V2)return;window.__BLIS_REFERENCE_V2=true;
...
const sources=()=>{try{return arr(window.S||S)}catch(_){return[]}};
const activity=()=>{try{return arr(window.A||A)}catch(_){return[]}};
...
function nav(){
  ...
  if(n.dataset.refv2Sig===sig)return;
...
function fallback(id){const h=host(id);if(!h)return;const label=NAV.find(x=>x[0]===id)?.[2]||'BLIS Navigator';h.innerHTML=head(label,`${clientName()} · ${clientSector()}`)+empty('Няма достатъчно данни за визуализация в този модул.')}
...
function boot(){
  ensurePages();syncGlobals();nav();
  const q=new URLSearchParams(location.search).get('page');const id=IDS.has(q)?q:(IDS.has(document.querySelector('.page.active')?.id)?document.querySelector('.page.active').id:'overview');
  activate(id);renderRoute(id);
  window.addEventListener('blis:clientdata',()=>requestAnimationFrame(()=>{syncGlobals();nav();window.BLISCanonicalRenderActive()}));
